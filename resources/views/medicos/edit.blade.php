    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-danger">
                ...
                <div class="box-body">
                    <form novalidate action="{{route('medicos.update',$medico->id)}}" method="POST" enctype="multipart/form-data">
                        {{csrf_field()}}
                                    ...
                                    <label class="control-label">Especialidades</label>
                                        @foreach($especialidades as $especialidade)
                                            <div class="form-check">
                                                <input
                                                    @foreach($medico->especialidades as $medico_especialidade)
                                                        @if($medico_especialidade->id == $especialidade->id)
                                                            checked
                                                        @endif
                                                    @endforeach name="especialidades[]"
                                                       class="form-check-input" type="checkbox" value="{{$especialidade->id}}"
